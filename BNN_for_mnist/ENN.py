 ...
 * @create date 2021-09-02 15:13:22
 * @modify date 2021-09-02 15:13:22
 * @desc [description]
 */
"""
...
class NN(nn.Module):
    ...
    def __init__(self, batch_size, hidden_1, hidden_2, output_size, z_mu, z_sigma):
    ...
    def forward(self, x):
        conv2_w_prior = nn.Parameter(torch.normal(mean=torch.zeros_like(self.conv2.weight)+self.z_mu, std=torch.ones_like(self.conv2.weight)*self.z_sigma))
        self.conv2.weight = nn.Parameter(conv2_w_prior*self.conv2_sigma + self.conv2_mu)
        conv2_2w_prior = nn.Parameter(torch.normal(mean=torch.zeros_like(self.conv2_2.weight)+self.z_mu, std=torch.ones_like(self.conv2_2.weight)*self.z_sigma))
        self.conv2_2.weight = nn.Parameter(conv2_2w_prior*self.conv2_2sigma + self.conv2_2mu)
        fc_prior = nn.Parameter(torch.normal(mean=torch.zeros_like(self.fc1.weight)+self.z_mu, std=torch.ones_like(self.fc1.weight)*self.z_sigma))
...
net = NN(32, 16, 64, 10, z_mu=0, z_sigma=0)
